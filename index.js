...
function TeamSpeakClient(host, port, name, call){
	events.EventEmitter.call(this);

	if (TS3.Clients[name]) {
		TS3.Clients[name].destroy();
		TS3.Clients[name] = null;
	}
	...
	function checkQueue(){
		if(!executing && queue.length >= 1){
			...
			TS3.Clients[name].write(executing.text + "\n");
		}
	}
	...
	// Check if there is an active connection on the server, before loggin
	TeamSpeakClient.prototype.checkClient = function(name) {
		return TS3.Clients[name];
	}
	...
	socket.on("lookup", function(err){
		self.emit("lookup", err);
	});
